...
def warc_to_html(warc_file_path, zipped=True, extracted=True, multi=True, dump=False):
    ...
    warc_file_path=os.path.join('files',warc_file_path)
...
def main():
    ...
    #warc_file_path = "CC-NEWS-20230101000546-03352.warc.gz"
    files='files'
    for warc_file_path in os.listdir(files):
        if warc_file_path[-8:]=='.warc.gz':
            # Set the desired options
            zipped = True  # Whether to zip the extracted files
            extracted = True  # Whether to keep the extracted files
            multi = True  # Whether to use multiprocessing
            dump = False  # Whether to store non-HTML files in a separate folder

            # Convert WARC to HTML
            extracted_folder = warc_to_html(warc_file_path, zipped, extracted, multi, dump)

            print("Extraction completed!")
            print("Extracted files stored in:", extracted_folder)
